...
"/Users/liuruilong/GitHub/BayYuan/Paddle/paddle/fluid/inference/mobilenet_ssd_hand",
"Directory of the inference model.");
DEFINE_string(combine_dirname,
"/home/parallels/Paddle/paddle/fluid/inference/fluid_fssd_new", "combine dir");
DEFINE_int32(repeat,
1, "Running the inference program repeat times");


template <typename T>
void GetInput(const std::string &input_name, paddle::framework::LoDTensor *input,
              paddle::framework::DDim dims) {
    auto *input_ptr = input->mutable_data<T>(dims, paddle::platform::CPUPlace());
    T *file_input_ptr = reinterpret_cast<T *>(malloc(sizeof(T) * input->numel()));
    std::ifstream in(input_name, std::ios::in | std::ios::binary);
    in.read(reinterpret_cast<char *>(file_input_ptr), input->numel() * sizeof(T));
    in.close();

    for (int i = 0; i < input->numel(); ++i) {
        input_ptr[i] = file_input_ptr[i];
    }
    free(file_input_ptr);
}
...
int main() {
    ...
    // mobilenet goodnet
//    SetupTensor<float>(&input, {FLAGS_batch_size, 3, 160, 160},static_cast<float>(0), static_cast<float>(1));
    GetInput<float>("/home/parallels/Paddle/paddle/fluid/inference/input_txt/003_0001.txt", &input, {FLAGS_batch_size, 3, 160, 160});
...
//            std::cout << " (index " << i << ": " << (*pTensor).data<float>()[i] << ") ";
//      }
