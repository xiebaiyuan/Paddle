...
DEFINE_string(dirname,
              "/Users/xiebaiyuan/PaddleProject/Paddle/paddle/fluid/inference/tests/mobilenet",
              "Directory of the inference model.");
DEFINE_string(combine_dirname,
              "/Users/xiebaiyuan/PaddleProject/Paddle/paddle/fluid/inference/tests/mobilenet_ssd_combined",
              "combine dir");
DEFINE_int32(repeat,
...
template<typename T>
void SetupTensorCv(paddle::framework::LoDTensor *inputTensor,
                   paddle::framework::DDim dims,
                   T lower,
                   T upper) {

//    cv::Mat srcImage = cv::imread("../test/images/hand.jpg");
//    cv::imshow("hand",srcImage);
//
//    wait(0);

    const char *picPath = "/Users/xiebaiyuan/PaddleProject/Paddle/paddle/fluid/inference/tests/paddle-mobile_modelsAndImages/images/banana.jpeg";

    float means[3] = {103.94f, 116.78f, 123.68f};
    float scale = 0.017f;
    ReadImage(picPath, &(*inputTensor), std::move(dims), means, scale, kBgr);

//    static unsigned int seed = 100;
//    std::mt19937 rng(seed++);
//    std::uniform_real_distribution<double> uniform_dist(0, 1);
//
//    auto *input_ptr = inputTensor->mutable_data<T>(dims, paddle::platform::CPUPlace());
//    std::cout << "inputTensor->numel() size = " << inputTensor->numel() << std::endl;
//    for (int i = 0; i < inputTensor->numel(); ++i) {
//        input_ptr[i] = static_cast<T>(uniform_dist(rng) * (upper - lower) + lower);
//        //   std::cout<< "index-"<<i<<" = "<<input_ptr[i]<<std::endl;
//    }
}
...
void TestInference(const std::string &dirname,
    ...
    paddle::platform::EnableProfiler(state);
    {
        ...
        std::cout << "iscombined: " << is_combined << std::endl;
        if (is_combined) {
...
// mobilenet ssd
    ...
    std::cout << "cpu_fetchs1 size: " << cpu_fetchs1.size() << std::endl;
